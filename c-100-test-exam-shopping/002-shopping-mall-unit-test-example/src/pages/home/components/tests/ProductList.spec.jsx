...
it('로딩이 완료된 경우 상품 리스트가 제대로 모두 노출된다', async () => {
  ...
  // findBy 는 getBy 와는 다르게 프로미스를 반환한다. 내부적으로 waitFor을 사용함
  // 1초 동안 50m 마다 요소가 있는지 확인
...
describe('로그인 상태일 경우', () => {
  const userId = 100;
  beforeEach(() => {
    mockUseUserStore({ isLogin: true, user: { id: userId } });
  });
  ...
  it('장바구니 버튼 클릭시 "장바구니 추가 완료!" toast를 노출하며, addCartItem 메서드가 호출된다.', async () => {
    ...
    expect(addCartItemFn).toHaveBeenNthCalledWith(1, product, userId, 1);
    expect(
