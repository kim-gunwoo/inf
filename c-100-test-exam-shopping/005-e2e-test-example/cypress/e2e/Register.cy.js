...
it('성공적으로 회원 가입이 완료되었을 경우 "가입 성공!"문구가 노출되며 로그인 페이지로 이동한다', () => {
  /**
   * stubbing: 특정 네트워크 요청에 대해 미리 정해진 응답을 반환하는것
   * intercept API는 요청과 응답에 대한 호출도 기록 -> stubbing 과 spying을 모두 실행할 수 있음
   */
  cy.intercept('POST', 'http://localhost:3000/users', { statusCode: 200 });

  cy.findByLabelText('이름').type('name');
  cy.findByLabelText('이메일').type('mock@email.com');
